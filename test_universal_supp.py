"""
Supplementary universal test entrypoint.

This entrypoint restores transfer-enabled routing (semantic/template/safe-switch)
for supplementary analysis while reusing strict mainline evaluation core.
"""
...
# Ensure importing test_universal does not trigger strict removed-flag preflight.
os.environ.setdefault("BAYES_PFL_TEST_ROUTE_PROFILE", "SUPPLEMENTARY_FALLBACK")
...
def validate_supplementary_args(args, parser: argparse.ArgumentParser) -> None:
    ...
    if all_disabled:
        parser.error(
            ...
            "For strict mainline runs, use test_universal.py instead."
        )
