...
class Makedataset():
	def __init__(self, train_data_path, preprocess_test, mode, image_size = 518,
				 allow_split_fallback=False, split_override=None, mask_guided_crop_prob=0.0):
		...
		self.mask_guided_crop_prob = mask_guided_crop_prob
	...
	def mask_dataset(self, name, product_list, batchsize, shuf = True, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=2):
		dataset = MyDataset(root=self.train_data_path, transform=self.preprocess_test, target_transform=self.target_transform,
											...
											allow_split_fallback=self.allow_split_fallback,
...
class MyDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, mode='train', dataset = "visa",
				 product_list = None, allow_split_fallback=False, split_override=None,
		...
		if mode in ("train", "val", "test"):
			...
			elif "test" in meta_root and allow_split_fallback:
				split_key = "test"
				import logging as _logging
				_logging.getLogger(__name__).warning(
					"Split '%s' not found in '%s', falling back to 'test' "
					"(allow_split_fallback=True)",
					effective_split, meta_path,
				)
			else:
				available_keys = ", ".join(meta_root.keys())
				raise KeyError(
					f"Split '{effective_split}' not found in '{meta_path}'. "
					f"Available keys: [{available_keys}]. "
					f"Use allow_split_fallback=True to fall back to 'test'."
				)
			meta_info = meta_root[split_key]
			if not isinstance(meta_info, dict):
				raise ValueError(
					f"Meta split '{split_key}' in '{meta_path}' should be a dict, got {type(meta_info)}"
				)

			if product_list is not None:
				product_set = set(product_list)
				meta_info = {k: v for k, v in meta_info.items() if k in product_set}
